<script type="text/javascript">
	...
	{
		window.location="<?php echo $_SERVER['PHP_SELF']?>?controller=master&action=news&function=edit&id="+id;	
	}
	
	function show_comfirm_del(id)
	{
		var comfirmBox;
		comfirmBox = confirm("Bạn có thật sự muốn xóa Chương trình này không?");
		
		if(comfirmBox == false){
			return false;
		}else{
			window.location="<?php echo $_SERVER['PHP_SELF']?>?controller=master&action=news&function=del&id="+id;	
		}
	}
...
<div class="label_content">
	<?php
    	if(isset($flag_search)){
			...
			echo "Danh sách chương trình khuyến mãi";	
		}
	?>
...
<form name="frmSearch" onsubmit="return Warning_search()" action="<?php echo $_SERVER['PHP_SELF'] ?>?controller=master&action=news&function=search" method="post">
    <div id="master_search" style="text-align:center; color: #000;">
        <h3>Tìm kiếm</h3><br />
        <img src="templates/01/images/network_utility128.png" /><br /><br />
        <div class="rdo">
                    <input type="radio" name="rdo_search" value="name" checked /> Tên chương trình &nbsp;
        </div>  
        <br />
		...
		<div id='show_cart'>
			<table>
				<tr id='head_showcart'>
					...
					<td style='width: 470px;'>Tên chương trình</td>
                ...
                ?>        
                        ...
                        <td>                        
                            ...
                            <input class="btn_del_one" type="button" onclick="show_comfirm_del('<?php echo $item['id']; ?>');" value="" title="Xóa chương trình" />
                        </td>
