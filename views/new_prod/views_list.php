<script type="text/javascript">
	...
	function show_comfirm_del_comment(id)
	{
		...
		if(comfirmBox == false){
			...
			window.location="<?php echo $_SERVER['PHP_SELF']?>?controller=master&action=comment_prod&function=del_comment_prod&id="+id;	
			//MASTER
		}
	}
</script>
...
<div class='black_color_form'>
	...
	echo "							
			<form name='frmComment' onsubmit='return Warning_comment()' action='". $url."gui-y-kien-tin-tuc.html' method='post' >
			...
			foreach($data_other as $item){
				if($flag >= 0 && $flag < 10 && $item['id'] != $data['id']){
					...
					echo "<img src='templates/01/images/bullet_green.png'>
						<a href='".$url."tin-tuc-moi-nhat/$tenctkm/$item[id].html'>$item[title]</a> 
